...
\cref{sec:intro} and~\ref{sec:background} established that three
...
\begin{enumerate}[label=(\roman*)]
  ...
  \item $|\Ztwenty| = 8$ (Golden Prime classes, \cref{eq:Z20star-card}),
...
generates~$i$) and the pentagon (which generates~$\golden$).  The non-circularity chain establishes a rigorous path from geometric invariants to spectral predictions: geometry (pentagon; $\golden = 2\cos(\pi/5)$) $\to$ algebra ($\golden^2 = \golden + 1$) $\to$ arithmetic (Artin character $\chi_5$; Galois functor $G$) $\to$ contraction ($|\ker G| = 2 \implies \mu = 1/2$) $\to$ spectral determination (unique roots $\mu = 1/2, \sigma = 3/2$). At no stage does any information from the Riemann zeros or the counting function~$N(T)$ enter the construction; the $T^*$ operator is a pure projection of the PCF arithmetic.
