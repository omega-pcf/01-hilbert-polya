...
\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{src/assets/placeholders/fig1_eigenvalues.png}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.9\textwidth]{src/assets/placeholders/fig4_gauss_eisenstein.png}
\caption{\textbf{Gauss--Eisenstein duality mediated by $\golden$.}
\emph{Left:} The tripartite operator $\OmH$ with eigenvalues
$\lambda_k = \tfrac{1}{2}\omega^k$ separated by $120^\circ$
(Eisenstein structure).
\emph{Right:} The rectangular lattice
$\Lambda_\mathrm{PCF} = \mathbb{Z}M \oplus \mathbb{Z}Mi$ with
$90^\circ$ fundamental cell (Gauss structure).
The arrow ``$\mathrm{PCF} \to$'' denotes the transformation
chain: Eisenstein~($120^\circ$) $\to$ scalar periodicity $\to$
Gauss~($90^\circ$). Golden mediation preserves the invariant
$\abs{\Omega} = 1/2$ while effecting the symmetry transition
$S_3 \to \mathbb{Z}_4$, generating the spectral parameters
$\mu = 1/2$, $\sigma = 3\mu = 3/2$.}
\label{fig:gauss-eisenstein}
\end{figure}

\begin{Remark}\label{rmk:gauss-eisenstein-not-new}
Equations~\eqref{eq:eisenstein-omega}
and~\eqref{eq:Omega-eigenvalues-restate} are not new definitions; they
re-identify~\eqref{eq:omega-root} and~\eqref{eq:Omega-hat-eigenvalues} in
the Gauss--Eisenstein context. No separate numbering is needed for the
proof chain; we keep them here only for the reader's convenience.
\end{Remark}

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{src/assets/placeholders/fig3_nine_panels.png}
\caption{\textbf{Golden mediation between Eisenstein and Gauss structures.}
Nine complementary views of the cylinder--lattice correspondence
through the coupling $z = \golden y$.
\emph{Row~1} (3D): cylinder $\mathcal{C}_0$ with equilateral
triangle $P$-$C$-$F$ (left); lattice $\Lambda_\mathrm{PCF}$ with
parallelogram fundamental cell (center); superposition showing the
triangle embedded in the lattice (right).
\emph{Row~2} (top views): circle $R_0 = 3$ with inscribed
$120^\circ$~triangle, Eisenstein structure (left); square cell
$M_\mathrm{PCF} \times M_\mathrm{PCF}$, Gauss structure (center);
superposition of circle-plus-triangle within square (right).
\emph{Row~3} (lateral and isometric): vertices aligned on $z = \golden y$
(left); lattice cell degenerating on $w = \golden v$ (center);
isometric view showing the fundamental cell as a rhombus (right).
The golden ratio~$\golden$ unifies both structures, preserving
$\abs{\Omega} = 1/2$ throughout.}
\label{fig:nine-panels}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{src/assets/placeholders/fig5_pentadimensional_tower.png}
\caption{\textbf{PCF pentadimensional lattice tower with
$\golden^\sigma$~scaling.}
\emph{Top left:} Lattice $\Lambda_\mathrm{PCF}$ with fundamental
cells $M_\sigma = M_\mathrm{PCF} \cdot \golden^\sigma$ at levels
$\sigma = 0,1,2,3$, showing self-similar growth by factor~$\golden$
between consecutive levels.
\emph{Top right:} Golden coupling projection: a square in
$(u,w)$~coordinates maps to a rectangle in
$(u, z + \golden w)$~coordinates, with vertical expansion by
factor~$\golden$ (Axiom~3).
\emph{Bottom left:} PCF hypercube in eight complementary views ---
four isometric perspectives (top row) and four orthogonal projections
(bottom row) --- revealing nested cubic structures scaling as $\golden^\sigma$.
\emph{Bottom right:} Full isometric view of the 3D tower
with the innermost cell ($\sigma = 0$) at the origin
and successive shells growing by~$\golden$. The discrete scale
dimension $\sigma \in \mathbb{Z}$ parametrizes the pentadimensional
structure $\mathcal{M}^5_\mathrm{PCF} = M^4 \times S^1_\sigma$.}
...
\]
Taking square roots: $d/s = \golden$. \qed
\end{proof}
