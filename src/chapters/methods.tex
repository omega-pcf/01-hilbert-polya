...
\]
Every structural claim is accompanied by its Lean~4 formalization
(\texttt{PCF\_Complete\_v11\_Unified.lean}: 646 lines, 122 declarations,
$0$~\texttt{sorry}).
...
\caption{\textbf{Gauss--Eisenstein duality mediated by $\golden$.} 
\emph{Left:} The tripartite operator $\OmH$ with eigenvalues 
$\lambda_k = \tfrac{1}{2}\omega^k$ separated by $120^\circ$ (Eisenstein structure). 
\emph{Right:} The rectangular lattice $\Lambda_{\mathrm{PCF}} = \mathbb{Z}M \oplus \mathbb{Z}Mi$ 
with $90^\circ$ fundamental cell (Gauss structure). The arrow ``$\mathrm{PCF} \to$'' 
denotes the transformation chain: Eisenstein ($120^\circ$) $\to$ scalar 
periodicity $\to$ Gauss ($90^\circ$). Golden mediation preserves the 
invariant $|\Omega| = 1/2$ while effecting the symmetry transition 
$S_3 \to \mathbb{Z}_4$, generating the spectral parameters $\mu = 1/2, \sigma = 3\mu = 3/2$.}
...
\caption{\textbf{Golden mediation between Eisenstein and Gauss structures (I: 3D views).} 
Three-dimensional views of the cylinder--lattice correspondence through the 
coupling $z = \golden y$. \emph{Left:} Cylinder $\mathcal{C}_0$ with 
equilateral triangle $P$-$C$-$F$. \emph{Center:} Lattice $\Lambda_{\mathrm{PCF}}$ 
with parallelogram fundamental cell. \emph{Right:} Superposition showing 
the triangle embedded in the lattice.}
\label{fig:cylinder-lattice-3d}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{src/images/cylinder_lattice_top}
\caption{\textbf{Golden mediation between Eisenstein and Gauss structures (II: Top views).} 
Planar projections of the correspondence. \emph{Left:} Circle $R_0 = 3$ 
with inscribed $120^\circ$ triangle (Eisenstein structure). 
\emph{Center:} Square cell $M_{\mathrm{PCF}} \times M_{\mathrm{PCF}}$ 
(Gauss structure). \emph{Right:} Superposition of circle-plus-triangle 
within square.}
\label{fig:cylinder-lattice-top}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{src/images/cylinder_lattice_lateral}
\caption{\textbf{Golden mediation between Eisenstein and Gauss structures (III: Lateral and isometric views).} 
Projective views revealing structural constraints. \emph{Left:} Vertices 
aligned on $z = \golden y$. \emph{Center:} Lattice cell degenerating on 
$w = \golden v$. \emph{Right:} Isometric view showing the fundamental 
cell as a rhombus. The golden ratio $\golden$ unifies both structures, 
preserving $|\Omega| = 1/2$ throughout.}
...
The discrete parameter $\sigma\in\mathbb{Z}$ generates a tower of
self-similar lattices $\Lambda_\sigma = \golden^\sigma\cdot\Lambda_{\mathrm{PCF}}$,
where each level inherits $|\Omega|=1/2$ and $S_3$ symmetry.  The
...
\caption{\textbf{PCF lattice growth and structural projection.}
\emph{Left:} Lattice $\Lambda_{\mathrm{PCF}}$ with fundamental cells $M_\sigma = M_{\mathrm{PCF}} \cdot \golden^\sigma$ at levels $\sigma = 0, 1, 2, 3$, showing self-similar growth by factor $\golden$ between consecutive levels.
\emph{Right:} Golden coupling projection: a square in $(u, w)$ coordinates maps to a rectangle in $(u, z + \golden w)$ coordinates, with vertical expansion by factor $\golden$ (Axiom~3).}
...
\caption{\textbf{PCF pentadimensional lattice tower with $\golden^\sigma$ scaling.}
\emph{Top row:} PCF hypercube in eight complementary views --- four isometric
perspectives --- revealing nested cubic structures scaling as $\golden^\sigma$. 
\emph{Bottom row:} Four orthogonal projections revealing the rigid cubic 
scaffolding. Full isometric view of the 3D tower with the innermost cell 
($\sigma = 0$) at the origin and successive shells growing by $\golden$. 
The discrete scale dimension $\sigma \in \mathbb{Z}$ parametrizes the 
pentadimensional structure $\mathcal{M}^5_{\mathrm{PCF}} = M^4 \times S^1_\sigma$.}
...
mean relative error ${\sim}1\%$ (bounded by ${<}1.7\%$) across twelve orders of magnitude
(\S\ref{sec:results}).  The structural parameters $d=3$, $\mu=1/2$,
$\sigma=3/2$ emerge from the PCF framework alone
(Proposition~\ref{prop:spectral-identities}), and the derivation is
formally verified in Lean~4 with zero \texttt{sorry} and axioms limited
to geometric constants of the PCF construction~(\S\ref{app:lean})
and Hecke's functional equation.
