...
\begin{proof}
The forgetful functor $U\colon C^*\text{--}\mathbf{Alg} \to \Hilb \to \Met \to \Set$ implements the arity transition, contracting the norm from $\norm{\cdot}_{\text{\textsc{alg}}} = 1$ ($\Set$ level) to $\norm{\Omega}_{\text{\textsc{pcf}}} = 1/2$ ($\Met$ level):
%
\begin{center}
...
At $\Set$: $\mu=1$ (no metric constraint); diagonal permitted (\cref{prop:set-level}).
At $\Hilb$: both norms coexist; diagonal is quadratic (\cref{prop:hilb-level}).
At $\Met$: $\mu=1/2<1$; diagonal blocked (\cref{prop:met-level}).
