...
unit in the group $\Ztwenty$.  In Lean: verified by \lean{fin\_cases} over
...
\begin{proof}
The ternary fragment is characterized by the spectral parameter $\mu_3 = 1/2$
...
\begin{proof}
The forgetful functor $U\colon C^*\text{--}\mathbf{Alg} \to \Hilb \to \Met$ contracts the norm from $\norm{\cdot}_{\text{\textsc{alg}}} = 1$ ($\Set$ level) to $\norm{\Omega}_{\text{\textsc{pcf}}} = 1/2$ ($\Met$ level):
%
\begin{center}
\begin{tikzcd}[column sep=3.5em, row sep=1.5em]
  \underset{\substack{\scriptstyle \text{binary, } \mu_2=1 \\ \scriptstyle \text{diagonal permitted}}}{C^*\text{--}\mathbf{Alg}}
    \arrow[r, "U_1"]
  & \underset{\scriptstyle \text{both norms}}{\mathbf{Hilb}}
    \arrow[r, "U_2"]
  & \underset{\substack{\scriptstyle \text{ternary, } \mu_3=1/2 \\ \scriptstyle \text{diagonal blocked}}}{\mathbf{Met}}
    \arrow[r, "U_3"]
  & \underset{\substack{\scriptstyle \text{no norm} \\ \scriptstyle \text{diagonal permitted}}}{\mathbf{Set}}
\end{tikzcd}
\end{center}
%
\noindent
...
(Dunford--Schwartz~\cite{dunford88}) and the Euler product identity.
...
Gelfand~(1960)~\cite{gelfand60}, Dunford--Schwartz~(1958)~\cite{dunford88},
...
\begin{enumerate}[label=(\arabic*), leftmargin=2.5em]
  \item $\OmH(k) = \tfrac{1}{2}\omega^k$ (eigenvalue formula, Eq.~\eqref{eq:Omega-hat-eigenvalues}).
  \item $|\OmH(k)| = 1/2$ for all $k$ (constant modulus, Eq.~\eqref{eq:modulus-Omega}).
  \item $\mathrm{Re}(\OmH(0)) = 1/2$ (critical line, Eq.~\eqref{eq:Omega-hat-0-re}).
  \item Unique positive-Re eigenvalue (Eq.~\eqref{eq:Omega-hat-unique-positive-re}).
