...
With the operator constructed (\S\ref{sec:methods}) and its predictions
verified against known zeros (\S\ref{sec:results}), we now interpret the
...
\[
  ...
  \Set.
\]
The \emph{arity transition} (Theorem~\ref{thm:arity-transition}) is a
...
% ------------------------------------------------------------------
\subsection{$\zeta$ as a Categorical Object}\label{ssec:zeta-cat}
% ------------------------------------------------------------------
...
destroyed (Proposition~\ref{prop:add-destroyed}).  The Euler product
therefore factors through the ternary level ($n=3$, $\mu_3=1/2$), where the
diagonal is blocked.

\begin{definition}[Golden residue group]\label{def:Z20star}
\begin{equation}\label{eq:Z20star}
  \Ztwenty = \{1,3,7,9,11,13,17,19\} \subset \mathbb{Z}/20\mathbb{Z}.
\end{equation}
\end{definition}

\begin{proposition}[Cardinality]\label{prop:Z20star-card}
\begin{equation}\label{eq:Z20star-card}
  |\Ztwenty| = 8.
\end{equation}
\end{proposition}
...
axiom.  Stronger: verified exhaustively for all $8\times 8 = 64$ pairs
by \lean{decide} in Lean.
...
Verified for all 64 pairs by \lean{decide}.
\qed
\end{proof}
...
fibers are as listed in Theorem~\ref{thm:golden-prime-classes}:
$\{1,9\}$, $\{11,19\}$, $\{13,17\}$, $\{3,7\}$.
\qed
\end{proof}

\begin{theorem}[Primes force ternary]\label{thm:zeta-forced}
\begin{equation}\label{eq:primes-in-Z20star}
...
$p\nmid 2$, $p\nmid 5$), so $p\bmod 20\in\Ztwenty$ by definition of the
unit group.  In Lean: verified by \lean{fin\_cases} over
...
\begin{equation}\label{eq:arith-fragment}
  \mathrm{ArithFragment} :=
    \mathrm{binary} \mid \mathrm{ternary}.
\end{equation}
The fragment of $\Ztwenty$ is ternary:
\begin{equation}\label{eq:Z20star-fragment}
...
\begin{equation}\label{eq:fragment-mu-ternary}
  \mathrm{fragment\_mu}(\mathrm{ternary}) = \tfrac{1}{2}.
\end{equation}
\end{proposition}

\begin{proof}
By definition, the ternary fragment has modulus $\mu_3 = 1/2$
(Eq.~\eqref{eq:mu-ternary}).
\qed
\end{proof}
...
by Theorem~\ref{thm:zeta-forced}.
\end{remark}
...
Definition~\ref{def:spectral-params} at the stated arity.
Verified by \lean{mu\_binary}, \lean{mu\_ternary},
\lean{sigma\_ternary} in Lean.
\qed
\end{proof}
...
(Definition~\ref{def:object-norm}) with
...
\norm{\Omega\otimes\Omega}=\norm{\Omega}^2$,
i.e., $\norm{\Omega}\le\norm{\Omega}^2$.  But $0<\norm{\Omega}<1$
contradicts Lemma~\ref{lem:diagonal-contraction}.
\qed
\end{proof}
...
Lemma~\ref{lem:diagonal-contraction}.
\qed
\end{proof}

\begin{remark}[Exponential objects and cartesian closure]%
\label{rmk:exponential-objects}
The argument in Proposition~\ref{prop:equiv-obstructions} is
...
Proposition~\ref{prop:equiv-obstructions}(c) blocks all such sections when
$\norm{\Omega}<1$.
\qed
\end{proof}
...
($\norm{\Omega}=\tfrac{1}{2}$, from~\eqref{eq:norm-Omega-half}) coexist.
...
$\mu=1/2<1$
(Eq.~\eqref{eq:mu-ternary}), Lawvere is blocked.
\end{proposition}
...
$\norm{\Omega}=1/2<1$.  Lemma~\ref{lem:diagonal-contraction}:
...
\begin{proof}
The forgetful functor $U\colon C^*\text{-}\mathbf{Alg}\to\Hilb\to\Met$
contracts the norm from $\norm{\cdot}_{\mathrm{alg}}=1$ ($\Set$ level)
to $\norm{\Omega}_{\mathrm{pcf}}=1/2$ ($\Met$ level):
%
\begin{center}
\begin{tikzcd}[column sep=2.8em, row sep=2em]
  ...
  \\[-0.5em]
  {\scriptstyle \mu_2=1}
  & {\scriptstyle \text{both norms}}
  & {\scriptstyle \mu_3=\frac{1}{2}}
  & {\scriptstyle \text{no norm}}
  \\[-0.5em]
  {\scriptstyle \text{diagonal OK}}
  &
  & {\scriptstyle \text{diagonal blocked}}
\end{tikzcd}
\end{center}
%
\noindent
The contraction factor is $\mu_2/\mu_3 = 1/(1/2) = 2 = |\ker G|$.
At $\Set$: $\mu=1$, diagonal permitted
(Proposition~\ref{prop:set-level}).
At $\Hilb$: both norms coexist, diagonal is quadratic
(Proposition~\ref{prop:hilb-level}).
At $\Met$: $\mu=1/2<1$, diagonal blocked
(Proposition~\ref{prop:met-level}).
\qed
\end{proof}
...
The forgetful functor $U\colon C^*$-$\mathbf{Alg}\to\Hilb$ carries two
coexisting norm levels:
\begin{align*}
  \text{Algebra unit norm:} &\quad \norm{\cdot}_{\mathrm{alg}} = 1
    && (\Set\text{ level, } \mu_2 = 1), \\
  \text{Element PCF norm:} &\quad \norm{\Omega}_{\mathrm{pcf}} = 1/2
    && (\Met\text{ level, } \mu_3 = 1/2).
\end{align*}
The strict inequality $\norm{\Omega}_{\mathrm{pcf}} < \norm{\cdot}_{\mathrm{alg}}$
is the contraction that blocks the diagonal; the contraction factor is
$1/2 = |\ker G|^{-1}$.
\end{definition}
...
\end{tabular}
\end{center}
Verified by \lean{G\_fibers\_partition} (\texttt{decide}) in Lean.
\end{definition}
...
$\mu_2/\mu_3=2$ (Proposition~\ref{prop:contraction-factor}).
...
Theorem~\ref{thm:zeta-forced} and $G$.
\end{theorem}

\begin{proof}
By Theorem~\ref{thm:zeta-forced}, every prime $p>5$ maps to $\Ztwenty$
under reduction modulo~$20$.  The Galois functor
$G\colon\Ztwenty\to\mathbb{Z}_2\times\mathbb{Z}_2$
(Theorem~\ref{thm:golden-prime-classes}) assigns to each factor
$(1-p^{-s})^{-1}$ of the Euler product a character value in
$\mathbb{Z}_2\times\mathbb{Z}_2$.  Since the group has exponent~$2$
(Proposition~\ref{prop:exponent-two}), all four characters are real and
...
limit of a real sequence (verified by \lean{Omega\_norm\_tendsto\_zero}
...
\begin{align}
  0 < \norm{\Omega}_k &< 1,
    \label{eq:Omega-norm-bounds}  \\
  \norm{\Omega}_{k+1} &< \norm{\Omega}_k
    \qquad\text{(strictly decreasing)},
    \label{eq:Omega-norm-decreasing}  \\
  \norm{\Omega}_k &\to 0 \quad\text{as } k\to\infty.
    \label{eq:Omega-norm-limit}
\end{align}
\end{lemma}
...
$0 < 1/2 < 1$.
Verified by \lean{Omega\_norm\_pos}, \lean{Omega\_norm\_lt\_one},
\lean{Omega\_norm\_decreasing}, \lean{Omega\_norm\_tendsto\_zero} in Lean.
\qed
\end{proof}
...
Theorem~\ref{thm:no-diagonal} applied to $\norm{\Omega}_k < 1$).
No retraction is possible at any level.
\end{corollary}

\begin{proof}
By Lemma~\ref{lem:tower-properties}, $0 < \norm{\Omega}_k < 1$ for
all~$k$.  By Theorem~\ref{thm:no-diagonal}, $\norm{\Omega}_k < 1$
...
\begin{equation}\label{eq:tower-base}
  ...
  \qquad\text{(Eq.~\eqref{eq:mu-ternary})}.
\end{equation}
\end{proposition}
...
(Eq.~\eqref{eq:mu-ternary}).
Verified by \lean{tower\_base\_is\_mu3} in Lean.
\qed
\end{proof}
...
\begin{proof}
Properties (L1)--(L3) and (L5) are Lemma~\ref{lem:tower-properties}
and Proposition~\ref{prop:tower-base}.  (L4) is the strict decrease from
Lemma~\ref{lem:tower-properties}.  (L6) is
Proposition~\ref{prop:spectral-uniqueness}.  (L7) is
Proposition~\ref{prop:add-destroyed}.  (L8) is
Theorem~\ref{thm:zeta-forced}.  The triple equality
$\mu_3 = \norm{\Omega}_0 = 1/2$ follows from (L5) and
Eq.~\eqref{eq:mu-ternary}.
Verified by \lean{categorical\allowbreak \_limit\allowbreak \_theorem} in Lean.%
\footnote{The Lean declaration retains the name
\texttt{categorical\allowbreak \_limit\allowbreak \_theorem} for backward compatibility with the
formalization files.}
\qed
\end{proof}
...
The eigenvalues of $\OmH$ were defined in~\eqref{eq:Omega-hat-eigenvalues}.
...
(Eq.~\eqref{eq:omega-root}) satisfies:
...
Verified by \lean{w\_properties} in Lean.
\qed
\end{proof}
...
Verified by \lean{Omega\_hat\_unique\_positive\_re} in Lean.
\qed
\end{proof}
...
\emph{Step~1: Euler product enters~$\Ztwenty$.}
The Euler product $\zeta(s)=\prod_p(1-p^{-s})^{-1}$ is a product over
primes.  Every prime $p>5$ satisfies $p\bmod 20\in\Ztwenty$
(Theorem~\ref{thm:zeta-forced}).  Therefore each factor of the Euler
product is indexed by an element of~$\Ztwenty$.

\emph{Step~2: $\Ztwenty$ forces the ternary fragment.}
The group $\Ztwenty$ is closed under multiplication but not under
addition (Proposition~\ref{prop:add-destroyed}).  This purely
multiplicative arithmetic selects the ternary fragment with
$\mu_3=1/2$ (Proposition~\ref{prop:fragment-mu}), uniquely determined
by the quadratic system $\sigma+\mu=2$, $\sigma\mu=3/4$
(Proposition~\ref{prop:spectral-uniqueness}).

\emph{Step~3: The ternary fragment has operator~$\OmH$.}
The ternary fragment is generated by the tripartite object
$\Omega = P\cdot C\cdot F$ with $\norm{\Omega}=1/2$
(Eq.~\eqref{eq:norm-Omega-half}), whose spectral decomposition is
$\OmH = \tfrac{1}{2}\,\mathrm{diag}(1,\omega,\omega^2)$
(Definition~\ref{def:Omega-hat}).  The No-Diagonal
Theorem~\ref{thm:no-diagonal} blocks self-reference at
$\norm{\Omega}<1$; the Arity Transition
(Theorem~\ref{thm:arity-transition}) identifies this contraction as
the forgetful functor $U\colon C^*$-$\mathbf{Alg}\to\Hilb\to\Met$
carrying the norm from~$1$ to~$1/2$.

\emph{Step~4: Zeros inherit the spectral constraint.}
The multiplicative structure of~$\Ztwenty$ groups Euler factors into
exactly four classes via the Galois functor
$G\colon\Ztwenty\to\mathbb{Z}_2\times\mathbb{Z}_2$
(Definition~\ref{def:galois-functor}), producing four real Dirichlet
characters $\tilde\chi_j$ (since $\mathbb{Z}_2\times\mathbb{Z}_2$ has
exponent~$2$, Proposition~\ref{prop:exponent-two}).  The tripartite $S_3$
...
parts are $\{1/2, -1/4\}$ (Proposition~\ref{prop:Omega-hat-re}).
Since each $L(s,\tilde\chi_j)$ factorizes through primes in~$\Ztwenty$
(step~1) and the fragment's spectral parameter is $\mu_3 = 1/2$ (step~2),
...
(steps~1--3) with the classical spectral mapping theorem
...
Gelfand~(1941)~\cite{gelfand41}, Dunford--Schwartz~(1958)~\cite{dunford58},
...
By Proposition~\ref{prop:spectral-embedding},
...
\\
(1) $\OmH(k) = \tfrac{1}{2}\omega^k$ (eigenvalue formula, Eq.~\eqref{eq:Omega-hat-eigenvalues}),
\\
(2) $|\OmH(k)| = 1/2$ for all $k$ (constant modulus, Eq.~\eqref{eq:modulus-Omega}),
\\
(3) $\mathrm{Re}(\OmH(0)) = 1/2$ (critical line, Eq.~\eqref{eq:Omega-hat-0-re}),
\\
(4) Unique positive-Re eigenvalue (Eq.~\eqref{eq:Omega-hat-unique-positive-re}),
\\
(5) $S_3$ symmetry: the three eigenvalues are related by $120^\circ$ rotations.
\end{theorem}

\begin{proof}
Properties (1)--(4) follow from the definitions
(Eqs.~\eqref{eq:Omega-hat-eigenvalues}, \eqref{eq:modulus-Omega},
\eqref{eq:Omega-hat-0-re}, \eqref{eq:Omega-hat-unique-positive-re}).
