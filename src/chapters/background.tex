...
strategies (Russell, Priest, Tarski); (iv)~convergence of
$\mathbb{F}_1$-geometry, string theory, and the P\'olya--Hilbert
...
Weil's proof, and the physical mechanisms that realize dimensional
transcendence---identifying at each stage the structural principles
that will be mathematically developed in~\S\ref{sec:methods}--\S\ref{sec:categorical}.
...
The fundamental idea of parameterizing families of geometric objects through equivalence relations---where distinct objects sharing structural properties are organized into parametric classes---has conceptual precedents that predate modern formalization. These historical precedents range from the classification of conics by Apollonius~\cite{apollonius} (c.~200~BCE), through the focus-directrix characterization with eccentricity parameters of Pappus~\cite{pappus} (c.~320~CE), to the projective perspective transformations of Alberti~\cite{alberti}, the systematization of orthogonal projections by Monge, and the formalization of isometric projection by Farish~\cite{farish22}. All these developments---classifications by geometric transformations or projections---anticipate the conceptual basis of modern moduli spaces.
...
millennia.  Marcus Vitruvius Pollio~\cite{vitruvius_de_arch}
(\emph{De Architectura},
c.~25~BC, Book~IV~\S3) defines the modulus as the semi-diameter of a
...
manifestation of the Vitruvian mechanism~\eqref{eq:triple} applied to
...
components of the Vitruvian triple~\eqref{eq:triple}.
...
geometry.  Gaspard Monge (1795) systematized three-dimensional
...
(\S\ref{ssec:frobenius}), they \emph{cannot} carry algebraic structure.
...
\emph{Ars Magna}~\cite{cardano} (1545) and the resolution of cubic equations, through
...
mechanism~\eqref{eq:triple}---is what provided the term ``modulus'' to
complex analysis.

The first mathematical deployment of ``modulus'' in number theory
appears in Gauss's \emph{Disquisitiones Arithmeticae}~\cite{gauss01} (1801), the last
...
evidence confirms Gauss's conscious borrowing from Vitruvius, the term
...
Jean-Robert Argand~\cite{argand06} (1806) unified the geometric and algebraic moduli.
...
disputed.  Schubring (cited in~\cite{mactutor_argand}) argues that
...
triple~\eqref{eq:triple}---center, radius (hands), angle
...
and categorically, the Vitruvian mechanism~\eqref{eq:triple}
transmitted through Renaissance horology.

The primitive structure of~$\mathbb{C}$ upon which complex
algebra was mounted is the triple~\eqref{eq:triple}: the same mechanism
...
Riemann~\cite{riemann1859} (1857, ``Theorie der Abel'schen Functionen'') generalized from
...
$\theta = \arg(z) \in [0, 2\pi)$.  Euler's formula
...
(ii)~Algebraic: $\mathbb{C} = \{x + iy : x, y \in \mathbb{R}\}$
...
Frobenius's theorem~\cite{frobenius} (1877) closed the question definitively: no
...
are reviewed in~\S\ref{sssec:frobenius}).  The temporal convergence is striking:
during the precise decades 1822--1877, geometry systematically
...
construction of~\S\ref{sec:methods} exploits.
...
within this constraint.  Von~Neumann's quantum mechanics (1927--32)
...
The introduction (\S\ref{ssec:circumvention}) identified three circumvention strategies
for the self-referential paradoxes that Lawvere--Yanofsky~\cite{lawvere69,yanofsky03} unified:
Russell's type stratification~\cite{priest79}, and Tarski's
decidable geometry~\cite{tarski48,tarski51}.  The connection between these strategies was
...
Tarski~\cite{tarski48,tarski51} (1948, 1951) showed that the first-order theory of
...
system can be constructed''~\cite[p.\,45]{tarski51}.  The boundary
...
decidable~\cite{presburger}.  The purely multiplicative theory of the
...
analysis of multiplicative arithmetic~\cite{skolem30}; see also
Mostowski's observation in Tarski~\cite[Note~5, p.\,51]{tarski51}
that elementary algebra, though lacking the general concept of an
integer, contains a decidable ``general arithmetic.''}.  It is their
...
in~$\Ztwenty$ (\S\ref{sec:categorical})---inhabits Tarski's decidable
fragment.  The Lawvere--Yanofsky diagonal (\S\ref{ssec:double-obstruction}) cannot form because
...
Teichm\"uller's moduli spaces~\cite{teichmuller} (\S\ref{ssec:C-structure}) received
...
with one element'' (Tits~\cite{soule04}) that exist as functors rather than classical
geometric objects.  Borger's~\cite{borger09} identification of $\Lambda$-ring
structures as $\mathbb{F}_1$-descent data (\S\ref{ssec:convergence})
...
in the introduction (\S\ref{ssec:circumvention}): the domain where Lawvere's diagonal
...
The forgetful functor of~\S\ref{ssec:decidability} implements a
...
The Huerta--Schreiber tower~\cite{huerta17} (\S\ref{ssec:convergence}) derives M-theory's dimensional
...
The AdS/CFT correspondence~\cite{maldacena97} (Maldacena, 1997) provides the most
concrete physical realization of the principle that Tarski identified
and Grothendieck formalized (\S\ref{sssec:grothendieck}).
...
equivalence~\cite{cds98}, introduced in~\S\ref{ssec:convergence}, connects toroidal
...
Frobenius (\S\ref{ssec:frobenius}) proves cannot exist as a division
algebra: it must be a dimension with discrete multiplicative scaling
($\golden$-mediated, as the construction of~\S\ref{sec:methods} will demonstrate) but
...
The introduction (\S\ref{ssec:double-obstruction}) established that the Riemann Hypothesis is
a theorem for function fields (Hasse~\cite{deligne74} 1974)
and that the aspiration to transport this proof
from~$\mathbb{F}_q$ to~$\mathbb{Z}$ motivates the
$\mathbb{F}_1$-program.  We develop here the \emph{technical
mechanism} of Weil's proof, because it is the template that the
categorical construction of~\S\ref{sec:categorical} reproduces.
...
\begin{enumerate}[label=(\roman*)]
  \item \textbf{The arithmetic surface.}  The proof requires
    ...
    geometry.  Manin's noncommutative tori~\cite{manin02} (\S\ref{ssec:convergence}) provide the
  ...
  \item \textbf{The Frobenius endomorphism.}  The proof requires
    ...
    $\Lambda$-ring structures (\S\ref{ssec:double-obstruction}) provide the characteristic-zero
  ...
  \item \textbf{The squeeze from intersection theory.}  The proof
    ...
    the categorical setting (\S\ref{sec:categorical}): two independent
...
construction of~\S\ref{sec:categorical} provides an explicit realization: two
...
The developments of \S\ref{ssec:modulus}--\S\ref{ssec:weil}
...
extracting the design principles for the construction of~\S\ref{sec:methods}.
...
Grothendieck's forgetful functor (\S\ref{sssec:grothendieck})
formalizes this principle categorically; Tarski's decidability
theorem (\S\ref{sssec:tarski}) justifies it logically.
...
Weil's proof (\S\ref{ssec:weil}) establishes the geometric
...
seeks to formalize~\cite{connes15}.
...
hexagonal to square.  The construction in Section~3 employs~$\golden$
...
bounds.  Lawvere's theorem requires the diagonal morphism
