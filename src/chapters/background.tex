...
a theorem for function fields (Hasse~\cite{hasse34}, Weil~\cite{weil48})
and that the aspiration to transport this proof
from~$\mathbb{F}_q$ to~$\mathbb{Z}$ motivates the
$\mathbb{F}_1$-program.  The proof for general varieties over finite fields (the Weil conjectures) was completed by Deligne~\cite{deligne74} using the \'etale cohomology framework developed by Serre~\cite{serre55} and Grothendieck~\cite{grothendieck1960elements}.  We develop here the \emph{technical
