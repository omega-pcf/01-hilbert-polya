...
\end{equation*}
Substituting the explicit form of $T^*(n)$ from Definition~\ref{def:Tstar} for Regime~II yields:
\begin{equation*}
  \frac{T^*(n)}{t_n} = \left( \frac{c(n) \cdot \pi n}{\ln(n/2+3/2)} \cdot \frac{\ln n}{2\pi n} \right) \cdot \frac{t_n^{\mathrm{RvM}}}{t_n}.
\end{equation*}
Simplifying the terms and grouping them by their functional origin (spectral modulation, kernel normalization, and Riemannic deficit) establishes the decomposition~\eqref{eq:bias-decomp}.
\end{proof}
...
(Verified in Lean~4, \S\ref{app:lean}).
