...
\\
\emph{(c)} \textbf{Parameter rigidity.}  Every parameter in $T^*(n)$ is structurally determined (\cref{ssec:non-circularity}). The values $\{\mu, \sigma, \alpha_0, \beta_0\}$ are not free variables adjusted to minimize spectral error, but unique consequences of the PCF geometry (e.g., the ternary contraction $|\ker G|=2$ and $S_3$ symmetry). The error profile is thus an emergent property of the interaction between fixed pentagonal invariants and the leading-order Riemann--von~Mangoldt asymptotics.
