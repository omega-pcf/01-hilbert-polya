...
as the sole external input (\cref{thm:hecke}, \cref{app:lean}).
...
Both results are established by the formal witnesses \lean{bound\allowbreak \_alone\allowbreak \_insufficient} and \lean{companion\allowbreak \_alone\allowbreak \_insufficient\allowbreak}.
\end{proof}
...
\begin{enumerate}
  \item \emph{Primes force $\Ztwenty$}
    (\cref{thm:zeta-forced}, \cref{eq:primes-in-Z20star}).
...
$1/2 \le \mu_3 = 1/2$ and $1 - 1/2 \le \mu_3 = 1/2$, as established by witness \lean{canonical\_zero}.
\end{proof}
...
\begin{enumerate}
  ...
  \item Component norms (Eqs.~\cref{eq:norm-P}--\cref{eq:norm-Omega-half}),
  \item Spectral parameters (Eqs.~\cref{eq:sigma-n}--\cref{eq:mu-n}),
...
assembled in \lean{master\_bridge\allowbreak \_v11}, 
...
\cref{prop:premise-A,prop:premise-B},
\cref{thm:rh-squeeze},~\ref{thm:rh-contradiction}), each
categorically derived.  The two bounds are provably independent
(\cref{prop:bounds-independent}).
\end{theorem}
...
(\cref{def:TernaryLZero}) eliminates this circularity entirely:
...
(\lean{rh\_squeeze}, 4~lines) verifies the result.
