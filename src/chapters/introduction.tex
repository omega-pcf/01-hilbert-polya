...
The conjectures attributed to David Hilbert~\cite{hilbert1935mathematische} and George P\'olya~\cite{polya19} in the
...
Hasse~\cite{hasse34} (1934) for elliptic curves and by Weil~\cite{weil48} (1948) for all curves
over finite fields, then extended to arbitrary varieties by
Deligne~\cite{deligne74} (1974).  This geometric proof for function fields was completed for $GL_n$ by the work of Drinfeld~\cite{drinfeld77} and Lafforgue~\cite{lafforgue02} using the Arthur--Selberg trace formula and the Lefschetz trace
...
case exists: the octonions~$\mathbb{O}$~(dimension~8), whose alternative algebraic structure was characterized by Zorn~\cite{zorn30} (1930) and which are
non-associative~\cite[Theorem~1]{baez02}.  The
Kervaire--Bott--Milnor theorem and Adams's Hopf invariant solution~\cite{adams60} (1960) completes the picture: \emph{all}
...
8 & Spectral embedding & Steps 1--7 $\to$ $\OmH$ & Prop.~\ref{prop:spectral-embedding} & \cref{ssec:omega-spectrum} \\
...
cohomological completion are discussed in~\cref{sec:discussion}.
