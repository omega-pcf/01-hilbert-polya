...
% -----------------------------------------------------------------
\subsection{The P\'olya operator: historical background and the circularity--\allowbreak periodicity problem}\label{ssec:polya-hist}\label{sec:circularity}
% -----------------------------------------------------------------

The conjectures attributed to David Hilbert and George P\'olya in the
...
montgomery73}.  Odlyzko's massive computational verification
...
(Hardy--Littlewood~\cite{hardy14}, Vinogradov~\cite{vinogradov58})
...
Connes's non-commutative geometry program~\cite{connes99,connes94}
...
spectral side of a Lefschetz-type trace formula~\cite{connes15}.
Lorscheid~\cite{lopezpena09} reformulated the product as
...
Hasse~(1934) for elliptic curves and by Weil~(1940) for all curves
over finite fields, then extended to arbitrary varieties by
Deligne~(1974) using the intersection form and Lefschetz trace
formula~\cite{milne15,oort14}.  Weil's proof exploits the geometric
...
$\mathbb{F}_1$-program~\cite{soule04,connes_consani08,lopezpena09}.
...
prohibition~(\S\ref{sssec:lawvere}), which excludes any operator
defined from its own spectral data; a dimensional
prohibition~(\S\ref{sssec:frobenius}), which excludes the
...
Frobenius~(1877) established that the only finite-dimensional
\emph{associative} division algebras over~$\mathbb{R}$ are
$\mathbb{R}$~(dimension~1), $\mathbb{C}$~(dimension~2),
and~$\mathbb{H}$~(dimension~4).  Hurwitz~(1898) extended this to
\emph{normed} division algebras, showing that exactly one additional
case exists: the octonions~$\mathbb{O}$~(dimension~8), which are
non-associative~\cite[Theorem~1]{baez02}.  The
Kervaire--Bott--Milnor theorem~(1958) completes the picture: \emph{all}
...
associativity~\cite[\S2.2]{baez02}.  Both properties---commutativity
...
Adler~\cite{adler95} developed quaternionic quantum mechanics
...
statistical property~\cite{adler95}.  No quaternionic or octonionic
...
effective complex structures~\cite{adler95,baez02} that GUE forces
...
\begin{enumerate}
  ...
  \item \textbf{Tarski's decidable geometry}~\cite{tarski48,tarskigivant99}:
...
% -----------------------------------------------------------------
\subsection{Convergence: $\mathbb{F}_1$-geometry, string theory,
and the P\'olya--Hilbert conjecture}\label{ssec:convergence}
% -----------------------------------------------------------------

This geometric--stratified escape connects directly to two distinct
programs within Manin's framework~\cite{manin13,manin02}: his
...
Geometry} (2002)~\cite{manin02}---and the arithmetic-differential program
...
$\mathbb{F}_1$~bridge~\cite{borger09,borger11,borger10}.  The
...
by Tits~\cite{tits57} in 1957, who observed that combinatorial
...
Soul\'e~\cite{soule04} (2004) proposed the first formal framework for
algebraic geometry over~$\mathbb{F}_1$, followed by
Connes--Consani~\cite{connes_consani08} (2008--2015), who connected it
...
Lorscheid~\cite{lopezpena09} provided comprehensive surveys of the
...
Within this program, Manin's contributions~\cite{manin13,manin02} are
...
geometry~\cite{borger11,borger10}.
...
that Connes's Weil strategy~\cite{connes15} identifies as necessary:
...
and Schwarz~\cite{cds98} demonstrated that toroidal compactification in
...
Schreiber~\cite{huerta17}, who demonstrated that M-theory's dimensional
...
compactification, and the P\'olya--Hilbert conjecture---converge with
...
The circumvention strategies of~\S\ref{ssec:circumvention} (Russell,
Tarski) define the escape route; the convergence
of~\S\ref{ssec:convergence} (Manin, Connes--Douglas--Schwarz,
...
We propose---and demonstrate in~\S\ref{sec:categorical}
...
These results are formally verified in Lean~4 with zero \texttt{sorry}
statements; axioms are limited to geometric constants of the PCF
construction~(\S\ref{ssec:axioms}) and Hecke's functional
equation~(\S\ref{ssec:squeeze}).
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{src/assets/placeholders/fig2_comparative.png}
\caption{\textbf{Comparative dimensional mechanisms.}
\textbf{(A)}~String theory worldsheet: the closed string $S^1$ sweeps
a cylinder $\Sigma = S^1 \times \mathbb{R}$, compactifying to
the torus~$T^2$.
\textbf{(B)}~AdS/CFT holography: bulk fields in $\mathrm{AdS}_{d+1}$
project to the conformal boundary $\mathrm{CFT}_d$ via $z \to 0$.
\textbf{(C)}~PCF extended space: the golden coupling $z = \golden y$
extends $\mathbb{C}$ to
$\Ecube = \{(x,y,z) \in \mathbb{R}^3 : z = \golden y\}$
with tripartite vertices $P$, $C$, $F$ satisfying $S_3$~symmetry.
\textbf{(D)}~PCF base cylinder $\mathcal{C}_0$:
vertices on $x^2 + y^2 = 9$ at $120^\circ$ separation,
with vertical heights $z = \golden y$ determined by golden coupling.
Unlike~(A) and~(B), the PCF mechanism~(C--D) requires no
presupposed Hamiltonian or holographic boundary; the third
coordinate emerges purely from $\golden^2 = \golden + 1$.}
...
pentagon $\to$ $\golden$ $\to$ $\chi_5$ $\to$ $G$ $\to$ $\Rpcf$
...
\begin{proof}
Two proofs are given in~\S\ref{ssec:omega-spectrum}
and~\S\ref{ssec:squeeze}.

\emph{First proof} (spectral, \S\ref{ssec:omega-spectrum}).
The Euler product places every prime in the ternary fragment
(Proposition~\ref{prop:spectral-embedding}); the spectral embedding
...
(Proposition~\ref{prop:Omega-hat-re}),
$\mathrm{Re}(\rho)=1/2$.  This proof does not require Hecke's
theorem.

\emph{Second proof} (squeeze, \S\ref{ssec:squeeze}).
Two independent bounds---$\mathrm{Re}(\rho)\le\mu_3$
(Proposition~\ref{prop:premise-A}, from the spectral embedding)
and $1-\mathrm{Re}(\rho)\le\mu_3$
(Proposition~\ref{prop:premise-B}, from Hecke's 1920 theorem)---together force
$\mathrm{Re}(\rho)=1/2$ by~\texttt{linarith}.  Neither bound alone
suffices (Proposition~\ref{prop:bounds-independent}).
This is the form formalized in Lean~4 with $0$~\texttt{sorry};
axioms limited to geometric constants of~\S\ref{ssec:axioms}
and Hecke's functional equation
(\lean{hecke\_1920}, Appendix~\ref{app:lean}).
\qed
\end{proof}
...
\begin{center}
\small
\begin{tabularx}{\textwidth}{r >{\hsize=0.6\hsize}X >{\hsize=1.2\hsize}X >{\hsize=1.2\hsize}X l}
\toprule
\# & Step & Method & Lean tactic & Source \\
\midrule
1 & Primes $\to$ $\Ztwenty$ & Euler product & \texttt{fin\_cases} & \S\ref{ssec:zeta-cat} \\
2 & Mul.\ closed in $\Ztwenty$ & Group axiom & \texttt{decide} (64 pairs) & \S\ref{ssec:zeta-cat} \\
3 & Addition destroyed & Parity & \texttt{decide} (64 pairs) & \S\ref{ssec:zeta-cat} \\
4 & Exponent 2 of $G$ & Galois structure & \texttt{decide} (4 elements) & \S\ref{ssec:zeta-cat} \\
5 & Fragment modulus $=1/2$ & Arity & \texttt{simp} & \S\ref{ssec:CM-cat} \\
6 & Spectral uniqueness & Quadratic system & \texttt{nlinarith} & \S\ref{ssec:CM-cat} \\
7 & No-Diagonal blocks & Norm obstruction & \texttt{linarith} & \S\ref{ssec:CM-cat} \\
8 & Spectral embedding & Steps 1--7 $\to$ $\OmH$ & Prop.~\ref{prop:spectral-embedding} & \S\ref{ssec:omega-spectrum} \\
9 & Upper bound: $\rho_{\mathrm{re}}\le 1/2$ & Step 8 & conjunction & \S\ref{ssec:squeeze} \\
10 & Companion bound: $1-\rho_{\mathrm{re}}\le 1/2$ & Steps 4+8+Hecke & Hecke 1920 & \S\ref{ssec:squeeze} \\
11 & Squeeze: $\rho_{\mathrm{re}}=1/2$ & Steps 9+10 & \texttt{linarith} & \S\ref{ssec:squeeze} \\
\bottomrule
\end{tabularx}
\end{center}

The spectral proof (Theorem~\ref{thm:rh-spectral}) reaches
...
\emph{What is proved.}  Within the PCF categorical framework---where
...
determined by arity uniqueness---Theorem~\ref{thm:main-intro}
establishes that every non-trivial zero of~$\zeta(s)$ satisfies
$\mathrm{Re}(\rho)=1/2$.  Two proofs are given: a spectral proof
(Theorem~\ref{thm:rh-spectral}, no classical input) and a squeeze
proof (Theorem~\ref{thm:rh-squeeze}, Hecke 1920).  The squeeze is
machine-verified in Lean~4
with $0$~\texttt{sorry}; axioms limited to geometric constants
of~\S\ref{ssec:axioms} and \lean{hecke\_1920}.
...
$n = 10^{12}$, Conjecture~\ref{conj:bounded-error}) is consistent
with the framework's scope.  The directions for
cohomological completion are discussed in~\S\ref{sec:discussion}.
...
In \textbf{Background} (\S\ref{sec:background}) we develop a detailed
...
In \textbf{Methods} (\S\ref{sec:methods}) we construct the Precedent-Current-Forthcoming Framework (PCF).  The generating mechanism is the golden
...
In \textbf{Results} (\S\ref{sec:results}) we provide computational
...
(Theorem~\ref{thm:peaked-bias}), and establish a global error bound
${<}1.7\%$ (Conjecture~\ref{conj:bounded-error}).

In \textbf{Categorical Implications} (\S\ref{sec:categorical}) we
...
verified---is formalized and fully compiled in Lean~4 with Mathlib
(\texttt{PCF\_Complete\_v11\_Unified.lean}: 646 lines, 122 declarations,
$0$~\texttt{sorry}; axioms limited to geometric constants
of~\S\ref{ssec:axioms} and \lean{hecke\_1920}).

\textbf{Discussion} (\S\ref{sec:discussion}) examines the
...
(\S\ref{ssec:hopf-pcf}), and connections to holographic and
string-theoretic structures.  Open problems are identified.

\textbf{Conclusions} (\S\ref{sec:conclusions}) summarizes contributions
