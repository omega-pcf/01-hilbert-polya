...
\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{src/images/fig2_comparative}
...
(Poincaré half-space) project to the conformal boundary $\mathrm{CFT}_d$ 
via the $z \to 0$ limit, implementing a structural information preservation 
across a lost dimension.
\textbf{(C)}~PCF extended space $E^3$: the golden coupling $z = \golden y$ 
(Axiom~3) extends $\mathbb{C}$ to 
$E^3 = \{(x,y,z) \in \mathbb{R}^3 : z = \golden y\}$ 
...
with vertical heights $z = \golden y$ determined by the golden relation 
$\golden^2 = \golden + 1$. Unlike conventional holographic boundaries, 
the third coordinate emerges purely from the algebraic coupling, 
avoiding the Frobenius division algebra obstruction.}
