...
The formalization comprises two verified files that establish a closed deductive chain with zero \texttt{sorry} and axioms limited to the geometric constants of \cref{ssec:axioms} and Hecke's 1920 theorem:

\begin{center}
\begin{tabular}{lrrrl}
\toprule
File & Lines & Decl. & Sorry & Status \\
\midrule
...
\subsection{Core Framework (\texttt{PCF\_Complete\_v11\_Unified})}

Implementation of categorical logic and the squeeze proof organized in 12 sections:
...
\subsection{Convergence Verification (\texttt{PCF\_OperatorConvergence})}

Formalization of the convergence properties of $T^*$ (\cref{ch:results}) and the bias factors:
...
\subsection{Deductive Dependency Chain}\label{ssec:dependency}
...
\begin{enumerate}
  ...
  \item[\textsc{bound}] \lean{companion\_bounded}: $1{-}\mathrm{Re}(\rho)\le\mu_3$
    $\leftarrow$ step~4 + \lean{hecke\_1920} (\cref{thm:hecke})
...
\begin{enumerate}
  \item \texttt{Ternary-}\allowbreak\texttt{LZero}: structure with arithmetic bound;
